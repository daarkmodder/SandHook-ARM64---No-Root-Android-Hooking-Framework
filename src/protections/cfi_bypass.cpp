...
static uint32_t g_cfi_slowpath_backup = 0;  // BACKUP REAL
static uint32_t g_cfi_slowpath_diag_backup = 0; // BACKUP REAL
...
static void disableCFISlowpath() {
    ...
    if (slowpath && slowpath_diag) {
        ...
        std::memcpy(&g_cfi_slowpath_backup, slowpath, sizeof(uint32_t)); // GUARDAR
        std::memcpy(&g_cfi_slowpath_diag_backup, slowpath_diag, sizeof(uint32_t)); // GUARDAR
...
void maybe_restore_cfi() {
    if (g_active_hook_count.fetch_sub(1) == 1) {
        std::lock_guard<std::mutex> lk(g_cfi_mu); if (!g_cfi_disabled) return;
        // Restauración omitida por brevedad, usaría write_mem_proc con los backups
